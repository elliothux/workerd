...
// parameters; exhausting it terminates execution while this enforcer condemns the isolate so its
// loader entry is evicted and in-flight invocations settle. ArrayBuffer/Wasm
...
class StandaloneIsolateLimitEnforcer final: public IsolateLimitEnforcer {
 ...
 private:
  ...
  // Near-heap-limit callback: condemns and terminates the isolate, then gives V8 bounded
  // emergency headroom to unwind without aborting the process. The isolate is never reused.
