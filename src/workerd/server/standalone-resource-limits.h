...
// Wraps an outbound channel so that every fetch-style request and tunneled connect() it
// dispatches on behalf of the current invocation acquires one simultaneous-connection slot
// first. The seventh and later concurrent operation queues before any side effect and proceeds
// when a slot is released; the lease is released when response headers arrive (send,
// acceptWebSocket, or CONNECT accept/reject), matching the public Standard definition. Transfer
// checks and tokens forward untouched. Apply to every dynamic Worker's outbound channels;
// channels of trusted platform Workers are never wrapped.
kj::Own<IoChannelFactory::SubrequestChannel> newConnectionAccountingChannel(
    kj::Own<IoChannelFactory::SubrequestChannel> inner);

// Attenuates a WorkerLoader capability inherited by a dynamic Worker. WorkerCode and
// entrypoint custom limits may narrow the containing immutable Version's dimensions, but a
// nested Worker can never widen them. Trusted static host loaders are not wrapped.
