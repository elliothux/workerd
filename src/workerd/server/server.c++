...
class Server::WorkerService final: public Service,
  ...
  kj::Own<WorkerInterface> createEntrypoint(IoChannelFactory::SubrequestMetadata metadata,
    ...
    KJ_IF_SOME(enforcerOverride, limitEnforcerOverride) {
      ...
      return newWorkerEntrypoint(threadContext, kj::atomicAddRef(*worker), entrypointName.clone(),
          ...
          metadata.fromPersistentStub, kj::mv(metadata.dynamicWorkerTails));
    }
 ...
 private:
  ...
  // Present when this dynamic worker runs under Standard resource limits: the shared
  // per-isolate state (condemnation, watchdog target) and the effective per-invocation
  // request limits materialized from the WorkerCode. Static workers are unlimited.
...
struct Server::WorkerDef {
  ...
  // Present when this dynamic worker declared Standard resource limits: the shared
  // per-isolate state used by the isolate and request limit enforcers. Static workers are
  // always unlimited.
  kj::Maybe<kj::Rc<StandaloneIsolateLimitState>> isolateLimitState;
 ...
 private:
  ...
  class WorkerStubImpl final: public WorkerStubChannel {
   ...
   private:
    ...
    // Present when the loaded WorkerCode declared Standard resource limits: the shared
    // per-isolate state backing condemnation and watchdog enforcement, and the effective
    // request limits materialized from the WorkerCode (no defaults are re-applied here).
    ...
    kj::Maybe<kj::Own<LimitEnforcer>> newInvocationEnforcer(
      ...
      KJ_IF_SOME(limits, entrypointLimits) {
        JSG_REQUIRE(requestLimits != kj::none, Error,
            "Dynamic Worker entrypoint limits require the WorkerCode to declare limits.");
    ...
    kj::Promise<void> start(Server& server,
      ...
      // Validate the declared Standard resource limits before compiling the child. An
      // omitted limits dictionary means the loader host made no budget declaration; a
      // present one is enforced natively (the per-request enforcers are created per
      // invocation by the entrypoint channels below).
      KJ_IF_SOME(l, source.limits) {
        auto effective = validateResourceLimits(l);
        limitState = kj::rc<StandaloneIsolateLimitState>();
        // Condemnation removes this isolate from the loader cache, so later invocations of
        // the same immutable Version key create a fresh isolate.
        KJ_ASSERT_NONNULL(limitState)->setEvictionCallback([this]() { onAbortIsolate(); });
        requestLimits = effective;
      }
      KJ_IF_SOME(ns, owner.tryGet()) {
...
kj::Promise<kj::Own<Server::WorkerService>> Server::makeWorkerImpl(kj::StringPtr name,
  ...
  // Dynamic workers that declared Standard resource limits run under the standalone isolate
  // enforcer (memory + startup CPU); everything else keeps the unlimited enforcer.
  ...
  auto linkCallback = [this, def = kj::mv(def), totalActorChannels](WorkerService& workerService,
    ...
    bool enforceConnections = def.isolateLimitState != kj::none;
