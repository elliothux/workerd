...
class Server::HostExtensionBroker final {
 public:
  HostExtensionBroker(kj::Own<kj::AsyncCapabilityStream> stream,
  ...
  kj::Promise<kj::Own<kj::AsyncCapabilityStream>> acquire(kj::String identity) {
    ...
    auto result = kj::newPromiseAndFulfiller<kj::Own<kj::AsyncCapabilityStream>>();
    // The broker stream is ordered. Keep reading this response even if the caller is canceled, or
    // the next acquire could consume the abandoned FD and attach it to the wrong identity.
    tasks.add(previous.then([this, identity = kj::mv(identity), done = kj::mv(turn.fulfiller),
                                result = kj::mv(result.fulfiller)]() mutable {
      ...
      return acquireForCaller(kj::mv(frame), kj::mv(result), kj::mv(done));
    }));
    return kj::mv(result.promise);
  }

 private:
  kj::Promise<void> acquireForCaller(kj::Array<byte> frame,
      kj::Own<kj::PromiseFulfiller<kj::Own<kj::AsyncCapabilityStream>>> result,
      kj::Own<kj::PromiseFulfiller<void>> done) {
    KJ_DEFER(done->fulfill());
    try {
      auto acquired = co_await acquireOne(kj::mv(frame));
      if (result->isWaiting()) result->fulfill(kj::mv(acquired));
    } catch (kj::Exception& exception) {
      if (result->isWaiting()) result->reject(kj::mv(exception));
    }
  }

  kj::Promise<kj::Own<kj::AsyncCapabilityStream>> acquireOne(kj::Array<byte> frame) {
    co_await stream->write(frame.asPtr());
    byte status;
    kj::OwnFd fd;
    auto result = co_await stream->tryReadWithFds(&status, 1, 1, &fd, 1);
    KJ_REQUIRE(result.byteCount == 1 && result.capCount == 1 && status == 0,
        "invalid host extension broker response");
    co_return lowLevelProvider.wrapUnixSocketFd(kj::mv(fd));
  }
...
void Server::enableHostExtensionBroker(
  ...
  hostExtensionBroker = kj::heap<HostExtensionBroker>(kj::mv(stream), lowLevelProvider, tasks);
}
