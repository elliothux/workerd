...
size_t StandaloneIsolateLimitEnforcer::onNearHeapLimit(
  ...
  // loader entry and settle every in-flight invocation of this isolate. V8 raises its usual
  // RangeError for the allocation that crossed the line; the isolate is never reused.
  enforcer.isolateState->takeCondemnedException(StandaloneLimitViolation::MEMORY);
  // Refuse to grow the limit so the crossing allocation fails.
