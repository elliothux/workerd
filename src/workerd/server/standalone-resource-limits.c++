...
class ConnectionAccountingChannel final: public IoChannelFactory::SubrequestChannel {
 public:
  ...
  kj::Own<WorkerInterface> startRequest(IoChannelFactory::SubrequestMetadata metadata) override {
    auto request = inner->startRequest(kj::mv(metadata));
    KJ_IF_SOME(context, IoContext::tryCurrent()) {
      return kj::heap<ConnectionAccountingWorkerInterface>(
          kj::mv(request), context.getLimitEnforcer());
    }

    // Platform-owned dispatch, such as delivering a completed trace to a tail Worker, can start
    // this transferred channel after the originating request context is gone. It is not tenant
    // outbound traffic and therefore does not consume that invocation's connection slots.
    return request;
  }
