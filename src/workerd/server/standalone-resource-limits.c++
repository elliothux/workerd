...
// Per-invocation simultaneous-connection accounting. Refcounted so open connections can
// outlive the RequestLimitEnforcer during IoContext teardown. All access is on the workerd
// event-loop thread.
...
class ConnectionAccountingWorkerInterface final: public WorkerInterface {
 public:
  ConnectionAccountingWorkerInterface(kj::Own<WorkerInterface> inner): inner(kj::mv(inner)) {}
  ...
  kj::Promise<void> connect(kj::StringPtr host,
    ...
    // connect stream buffers until accept(), so queueing produces no side effect. Host and
    // header references stay valid until this handler's promise settles (see
    // kj/compat/http.c++; workerd's own output-gate deferral relies on the same guarantee).
    auto& ioContext = IoContext::current();
    return ioContext.getLimitEnforcer().newOutboundConnection().then(
        [inner = kj::mv(inner), host = kj::str(host), headers = headers.clone(), &connection,
            &response, settings](
            kj::Own<LimitEnforcer::OutboundConnectionLease> lease) mutable -> kj::Promise<void> {
      // The lease releases the slot when the pump settles or this promise is cancelled.
      return inner->connect(host, headers, connection, response, kj::mv(settings))
          .attach(kj::mv(lease), kj::mv(inner));
    });
  }
...
class ConnectionAccountingChannel final: public IoChannelFactory::SubrequestChannel {
 public:
  ...
  kj::Own<WorkerInterface> startRequest(IoChannelFactory::SubrequestMetadata metadata) override {
    return kj::heap<ConnectionAccountingWorkerInterface>(inner->startRequest(kj::mv(metadata)));
  }
...
void StandaloneIsolateLimitState::condemnFromWatchdog(StandaloneLimitViolation violationParam) {
  condemned.store(true, std::memory_order_acq_rel);
